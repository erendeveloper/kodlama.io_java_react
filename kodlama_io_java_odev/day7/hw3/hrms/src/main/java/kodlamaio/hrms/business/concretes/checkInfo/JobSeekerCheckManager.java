...
public class JobSeekerCheckManager{
    ...
    public boolean checkJobSeekerInfo(String rePassword, List<JobSeeker> allJobSeekers){
        ...
        if(!checkEmptyFields(rePassword))
            return false;
        if(!jobSeeker.getPassword().equals(rePassword)){
            message="Passwords does not match";
            return false;
        }
        if (!CheckMernis.checkJobSeeker(jobSeeker)){
            message="Jobsseker data was not verified by Mernis";
            return false;
        }
        if(checkNationalityIdSaved(allJobSeekers)){
            return false;
        }
        if(checkEmailSaved(allJobSeekers)){
    ...
    public boolean checkNationalityIdSaved(List<JobSeeker> allJobSeekers){
    ...
    public boolean checkEmailSaved(List<JobSeeker> allJobSeekers){
        for(JobSeeker jobSeeker : allJobSeekers){
